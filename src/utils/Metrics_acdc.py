...
def load_nrrd(img_path):
    ...
    return sitk.GetArrayFromImage(sitk_img), None, list(reversed(sitk_img.GetSpacing()))
...
def load_nii(img_path):
    ...
    return nimg.get_data(), nimg.affine, nimg.header.get_zoom()
...
def compute_metrics_on_directories(dir_gt, dir_pred, results_path = "results_{}.csv".format(time.strftime("%Y%m%d_%H%M%S"))):
    """
    ...
    for p_gt, p_pred in zip(lst_gt, lst_pred):
        ...
        # use the gt spacing (zooms) to calculate the volume per label
        gt, _, zooms = load_nrrd(p_gt)
        pred, _, _ = load_nrrd(p_pred)
