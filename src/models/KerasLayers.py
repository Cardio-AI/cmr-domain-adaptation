...
__all__ = ['UpSampling2DInterpol', 'UpSampling3DInterpol', 'Euler2Matrix', 'ScaleLayer',
           'UnetWrapper', 'ConvEncoder', 'conv_layer', 'downsampling_block', 'upsampling_block', 'Inverse3DMatrix', 'ConvDecoder', 'ConvEncoder']
...
class Euler2Matrix(Layer):
    ...
    def call(self, theta):
        ...
        # clip values in a range -pi to pi
        pi = tf.constant(m.pi)
...
class UnetWrapper(Layer):
    ...
    def call(self, x):
        ...
        #x = tf.transpose(x, [1, 0, 2, 3,4])
        # approximation of the unet prediction, which is trained in full resolution
        #if tf.shape(x)[-2] < self.unet_inplane[0] or tf.shape(x)[-2] > self.unet_inplane[0]:
...
class UpSampleBlock(Layer):
    def __init__(self, use_upsample=True, filters=16, f_size=(3, 3, 3), activation='elu',
        ...
        self.convTranspose = ConvTranspose(filters=self.filters, kernel_size=f_size, strides=self.up_size, padding=self.pad,
                                    kernel_initializer=self.kernel_init,
                                    activation=self.activation)
